...
两条路径的输出均遵守[对象 Web URL 证据合同](../../zentao/references/web-urls.md)：
不得仅凭 ID 或历史示例拼接页面链接。没有可靠来源时只报告 Bug ID 及
“页面 URL：当前能力无法可靠生成/尚未验证”；这不是 lifecycle 阻塞条件，
不为第 0 节增加页面探测或业务证据门槛。
