...
from ...services.feedbacks.service import FeedbacksService
...
def register(resource_subparsers: argparse._SubParsersAction[argparse.ArgumentParser]) -> None:
    p_create = resource_subparsers.add_parser('create', help='创建反馈')
    ...
    p_create.add_argument('--type', type=str, choices=['advice', 'bug', 'issue', 'opportunity', 'risk', 'story', 'task', 'todo'], dest='type', help='type 参数')
    ...
    p_create.add_argument('--feedback-by', type=str, dest='feedback_by', help='feedback-by 参数')
    p_create.add_argument('--source', type=str, dest='source', help='source 参数')
    add_json_flag(p_create)
    p_create.set_defaults(_handler=_run_create)
    p_edit = resource_subparsers.add_parser('edit', help='修改反馈')
    ...
    p_edit.add_argument('--type', type=str, choices=['advice', 'bug', 'issue', 'opportunity', 'risk', 'story', 'task', 'todo'], dest='type', help='type 参数')
    ...
    p_edit.add_argument('--feedback-by', type=str, dest='feedback_by', help='feedback-by 参数')
    p_edit.add_argument('--source', type=str, dest='source', help='source 参数')
    add_json_flag(p_edit)
    p_edit.set_defaults(_handler=_run_edit)
    p_list = resource_subparsers.add_parser('list', help='获取产品反馈列表')
    ...
    p_close.add_argument('--confirm-close', type=str, dest='confirm_close', help='confirm-close 参数')
    add_json_flag(p_close)
    p_close.set_defaults(_handler=_run_close)
    p_activate = resource_subparsers.add_parser('activate', help='激活反馈')
    ...
    p_delete = resource_subparsers.add_parser('delete', help='删除反馈')
...
def _run_create(services: object, args: argparse.Namespace) -> object | None:
    service: FeedbacksService = getattr(services, 'feedback')
    value_type = getattr(args, 'type', None)
    if value_type is not None and value_type not in {'risk', 'todo', 'advice', 'task', 'opportunity', 'story', 'issue', 'bug'}:
        raise UsageError("--type 不是当前 endpoint 支持的枚举值")
    return service.create(product=getattr(args, 'product', None), title=getattr(args, 'title', None), module=getattr(args, 'module', None), type=getattr(args, 'type', None), desc=resolve_text(args, 'desc'), feedback_by=getattr(args, 'feedback_by', None), source=getattr(args, 'source', None))
...
def _run_edit(services: object, args: argparse.Namespace) -> object | None:
    ...
    if value_type is not None and value_type not in {'risk', 'todo', 'advice', 'task', 'opportunity', 'story', 'issue', 'bug'}:
        raise UsageError("--type 不是当前 endpoint 支持的枚举值")
    return service.edit(item_id=args.id, product=getattr(args, 'product', None), title=getattr(args, 'title', None), module=getattr(args, 'module', None), type=getattr(args, 'type', None), desc=resolve_text(args, 'desc'), feedback_by=getattr(args, 'feedback_by', None), source=getattr(args, 'source', None))
...
def _run_close(services: object, args: argparse.Namespace) -> object | None:
    ...
    return service.close(item_id=args.id, closed_reason=getattr(args, 'closed_reason', None), comment=resolve_text(args, 'comment'), confirm_close=getattr(args, 'confirm_close', None))
