...
from ...services.epics.service import EpicsService
from ..common import add_json_flag, auto_value, number, page_int, per_page_int, positive_int, resolve_text


ENDPOINT_IDS = frozenset({'epic.list_product', 'epic.create', 'epic.activate', 'epic.view', 'epic.edit', 'epic.delete', 'epic.close', 'epic.change'})

def register(resource_subparsers: argparse._SubParsersAction[argparse.ArgumentParser]) -> None:
    p_create = resource_subparsers.add_parser('create', help='创建业务需求')
    ...
    p_edit = resource_subparsers.add_parser('edit', help='修改业务需求')
    ...
    p_change = resource_subparsers.add_parser('change', help='变更业务需求')
    ...
    p_list = resource_subparsers.add_parser('list', help='获取产品业务需求列表')
    ...
    p_view = resource_subparsers.add_parser('view', help='获取业务需求详情')
    p_view.add_argument("id", type=positive_int, help="资源 ID")
    add_json_flag(p_view)
    p_view.set_defaults(_handler=_run_view)
    p_close = resource_subparsers.add_parser('close', help='关闭业务需求')
    ...
    p_close.add_argument('--duplicate-story', type=positive_int, dest='duplicate_story', help='重复业务需求 ID；closed-reason=duplicate 时按目标要求提供')
    ...
    p_activate = resource_subparsers.add_parser('activate', help='激活业务需求')
    ...
    p_delete = resource_subparsers.add_parser('delete', help='删除业务需求')
...
def _run_create(services: object, args: argparse.Namespace) -> object | None:
    service: EpicsService = getattr(services, 'epic')
    ...
    return service.create(product=getattr(args, 'product', None), title=getattr(args, 'title', None), priority=getattr(args, 'priority', None), module=getattr(args, 'module', None), parent=getattr(args, 'parent', None), estimate=getattr(args, 'estimate', None), spec=resolve_text(args, 'spec'), category=getattr(args, 'category', None), source=getattr(args, 'source', None), verify=resolve_text(args, 'verify'), assignee=getattr(args, 'assignee', None), reviewer=getattr(args, 'reviewer', None))
