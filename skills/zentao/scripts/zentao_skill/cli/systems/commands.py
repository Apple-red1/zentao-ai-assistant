...
def register(resource_subparsers: argparse._SubParsersAction[argparse.ArgumentParser]) -> None:
    ...
    p_create.add_argument('--integrated', type=int, choices=(0, 1), required=True, dest='integrated', help='integrated 参数（0 否，1 是）')
    p_create.add_argument('--child', action='append', default=[], dest='child', help='child 参数；非集成应用可省略，发送空数组')
    ...
    p_edit.add_argument('--child', action='append', default=[], dest='child', help='child 参数；无子应用可省略，发送空数组')
