...
from ..common import add_json_flag, auto_value, build_ref, non_negative_int, number, page_int, per_page_int, positive_int, resolve_multiline_text, resolve_text
...
def register(resource_subparsers: argparse._SubParsersAction[argparse.ArgumentParser]) -> None:
    ...
    p_create_g_steps.add_argument('--steps-json', dest='steps_json', help="读取一个 JSON 编码的 steps 字符串，只解码一次")
    ...
    p_edit_g_steps.add_argument('--steps-json', dest='steps_json', help="读取一个 JSON 编码的 steps 字符串，只解码一次")
