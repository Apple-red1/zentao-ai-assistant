...
def register(resource_subparsers: argparse._SubParsersAction[argparse.ArgumentParser]) -> None:
    ...
    p_list = resource_subparsers.add_parser('list', help='获取项目集列表')
    ...
    p_view = resource_subparsers.add_parser('view', help='获取项目集详情')
    p_view.add_argument("id", type=positive_int, help="资源 ID")
    add_json_flag(p_view)
    p_view.set_defaults(_handler=_run_view)
    p_delete = resource_subparsers.add_parser('delete', help='删除项目集')
...
def _run_create(services: object, args: argparse.Namespace) -> object | None:
    service: ProgramsService = getattr(services, 'program')
    return service.create(name=getattr(args, 'name', None), begin=getattr(args, 'begin', None), end=getattr(args, 'end', None), pm=getattr(args, 'pm', None), desc=resolve_text(args, 'desc'))

def _run_edit(services: object, args: argparse.Namespace) -> object | None:
    service: ProgramsService = getattr(services, 'program')
    return service.edit(item_id=args.id, name=getattr(args, 'name', None), begin=getattr(args, 'begin', None), end=getattr(args, 'end', None), pm=getattr(args, 'pm', None), desc=resolve_text(args, 'desc'))
