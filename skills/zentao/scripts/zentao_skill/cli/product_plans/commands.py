...
def register(resource_subparsers: argparse._SubParsersAction[argparse.ArgumentParser]) -> None:
    ...
    p_edit.add_argument('--branch', type=positive_int, dest='branch', help='branch 参数')
...
def _run_edit(services: object, args: argparse.Namespace) -> object | None:
    service: ProductPlansService = getattr(services, 'product_plan')
    return service.edit(item_id=args.id, title=getattr(args, 'title', None), parent=getattr(args, 'parent', None), begin=getattr(args, 'begin', None), end=getattr(args, 'end', None), branch=getattr(args, 'branch', None), desc=resolve_text(args, 'desc'))
