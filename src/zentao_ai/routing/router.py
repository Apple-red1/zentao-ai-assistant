...
def route_bug(snapshot: BugSnapshot, config: AppConfig) -> RoutingDecision:
    ...
    matches = front + back
    exact_keys = [key for key in config.repositories if snapshot.scope and unicodedata.normalize("NFC", key).strip().casefold() == unicodedata.normalize("NFC", snapshot.scope).strip().casefold()]
