---
name: zentao-ai-bug
description: Use when 需要处理禅道 Zentao 个人 Bug、进行全局不限定负责人 Bug 标题关键词查询、生成团队或每日 Bug 日报、自动添加补充信息评论，或在严格安全门槛下辅助修改代码。
---
...
用于个人未关闭 Bug 的发现、分析、受控代码辅助和个人日报，也用于全局不限定负责人 Bug 标题关键词只读查询，以及组长按配置成员生成只读团队日报。它编排真实研发流程，不是 MCP API 的简单转发层。
...
4. 进入证据阶段前，从本 Skill 目录运行 `python ../../scripts/direct-branch-guard.py preflight`，校验唯一仓库、仓库租约、`codeWriteEnabled`、受保护分支、干净状态、上游和本地引用 `ahead/behind=0/0`。分支名比较不区分大小写：以 `dev`、`test` 或 `release` 开头，以及精确等于 `main` 或 `master` 时拒绝；`feature/main-fix` 等其他分支不因名称被拒绝。任一失败立即停止代码副作用。
