...
- 统计、个人与项目管理的关键数量由脚本确定性计算；`complete/partial_failures` 必须保留。
...
API 专项仍必须输出各 surface `120/120`、`Real API calls: 0` 和 `Result: PASS`；官方 evidence 与 21.7.8 compatibility 单独报告。
