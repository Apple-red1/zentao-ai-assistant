...
`bug create/edit --steps-inline-image` 只接受本地 png、jpg、jpeg、gif 或 webp 文件，并在
建立页面会话前完成路径校验。实现只使用仓库构造的同源 `bug/create`、`bug/edit` 和
`file/ajaxUpload` 页面路由；表单只按精确字段名读取 `uid`，兼容 `hidden` 与真实页面的
`text` 类型，拒绝把其它可见输入自动纳入 payload，且对缺失、空值或冲突控件继续
fail-closed。uid 绑定图片临时上传，用户 `steps` 文本先 HTML 转义，只有受控的 `<img src>`
片段进入步骤字段，不接受任意 HTML、外部 URL 或评论回退。

上传响应必须包含有效 file ID 和同源无凭据 URL；权限/能力失败发生在业务写入前时不创建或
修改 Bug。创建/编辑只提交一次页面表单，随后通过 API 详情确认步骤引用和 file 的 Bug
归属；确认失败不重试。上传或页面写入的网络结果不确定时返回 `UNKNOWN_WRITE_RESULT`，
并标记可能的孤儿上传；不重传、不转写评论、不自动删除。

聊天附件路由属于同一安全边界：用户明确要求图片进入 Bug 描述/重现步骤时，只能把宿主
提供且可读的本地路径传给 `bug create/edit --steps-inline-image`。不得把该图片改写为
`bug comment --inline-image` 或普通附件；宿主无法提供本地路径时必须在任何 Bug 写入前
阻塞。独立评论仅由用户明确的“添加备注/历史记录”意图触发，避免把图片位置错误伪装成
成功。

## 独立评论与内嵌图片

- 评论只注册在已验证的十种对象类型上；普通附件只在 Bug/Story 接受，内嵌图片只在 Bug
  接受且可重复提交。参数校验在建立页面会话前完成，拒绝空正文、非法路径、附件与图片
  混用和不支持的对象能力。
...
`zentao-personal` 名单维护是本机配置写入，必须由当前用户明确要求。文件位于
用户 `teams/`，按规范化站点和账号隔离，不放入连接 `.env`、项目临时目录或宿主
cache，不保存密码/Token。成员写入前完整读取真实用户目录，重名/缺失/冲突或
分页不完整时不修改名单。目录/文件 0700/0600，拒绝符号链接与非普通文件；
未知版本或损坏配置不自动重置，独占锁和原子替换避免并发覆盖与半文件。
只读查询不创建名单；团队查询仍只调用现有读取 endpoint，链接生成不访问页面。
