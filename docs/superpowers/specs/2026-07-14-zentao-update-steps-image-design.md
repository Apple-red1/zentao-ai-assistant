...
为本机自写 `zentao-mcp-server` 增加受控的 Bug 重现步骤编辑与图片上传能力，并同步更新本机 `zentao-ai-bug` Skill。合成目标场景是把用户明确提供的图片作为“图1”插入 `BUG-1001` 的 `[步骤]` 内容。
...
- Skill 静态契约测试全部通过。
- 不在测试中访问真实禅道或上传真实文件。
- 实际更新合成示例 `BUG-1001` 前重新取得用户对具体步骤正文与图片的操作确认。
