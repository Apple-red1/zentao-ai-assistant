...
对象详情链接遵守[对象 Web URL 证据合同](skills/zentao/references/web-urls.md)：
不得仅凭 ID、base URL 或历史示例拼接页面链接。没有当前实例的可靠链接证据时，
返回对象 ID 及“页面 URL：当前能力无法可靠生成/尚未验证”。当前没有通用 Web URL
生成/验证命令，API 查询成功不代表页面可访问。
