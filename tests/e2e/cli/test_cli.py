...
from zentao_ai.cli.runtime import AppRuntime, DependencyFactory
...
class Provider:
    ...
    def query_my_bugs(self, *, scope_names: tuple[str, ...], page: int, page_size: int) -> BugPage:
...
def factory(tmp_path: Path, provider: Provider | None = None, store: Store | None = None) -> DependencyFactory:
    runtime = AppRuntime(CONFIG, provider or Provider(), object(), lambda: None, "test", store=store or Store())
    return DependencyFactory(lambda _project: runtime)


def test_help_exposes_complete_command_tree() -> None:
    result = CliRunner().invoke(app, ["--help"])
    assert result.exit_code == 0
    for command in ("config", "auth", "doctor", "bugs", "report", "bug", "repair", "run", "mcp"):
...
def test_config_init_refuses_existing_then_force_replaces(tmp_path: Path) -> None:
    ...
    runner = CliRunner()
    denied = runner.invoke(app, ["config", "init", "--path", str(target)], input="demo\n")
    assert denied.exit_code == 2
    assert target.read_text(encoding="utf-8") == "old"
    replaced = runner.invoke(app, ["config", "init", "--path", str(target), "--force"], input="demo\n")
...
def test_auth_login_hides_and_stores_secret(tmp_path: Path) -> None:
    store = Store()
    result = CliRunner().invoke(app, ["auth", "login"], input="top-secret\n", obj=factory(tmp_path, store=store))
    assert result.exit_code == 0
...
def test_bugs_mine_uses_injected_provider_scope_and_json(tmp_path: Path) -> None:
    provider = Provider()
    result = CliRunner().invoke(app, ["bugs", "mine", "--json"], obj=factory(tmp_path, provider=provider))
    assert result.exit_code == 0
    assert json.loads(result.stdout)["items"][0]["id"] == 7
    assert provider.calls == [("mine", ("demo",), 1, 20)]


def test_run_dry_run_only_prints_ordered_plan(tmp_path: Path) -> None:
    result = CliRunner().invoke(app, ["run", "--dry-run", "--json"], obj=factory(tmp_path))
    assert result.exit_code == 0
    payload = json.loads(result.stdout)
    assert payload["executed"] is False
    assert payload["operations"] == ["query_my_bugs", "query_bug_detail", "query_bug_history"]
...
def test_provider_exception_is_redacted_and_business_exit_three(tmp_path: Path) -> None:
    ...
    result = CliRunner().invoke(app, ["bugs", "mine"], obj=factory(tmp_path, provider=FailingProvider()))
    assert result.exit_code == 3
    assert "top-secret" not in result.output


def test_auth_cancel_exits_130_without_store_write(tmp_path: Path, monkeypatch: object) -> None:
    import typer

    store = Store()
    monkeypatch.setattr(typer, "prompt", lambda *_args, **_kwargs: (_ for _ in ()).throw(typer.Abort()))  # type: ignore[attr-defined]
    result = CliRunner().invoke(app, ["auth", "login"], obj=factory(tmp_path, store=store))
