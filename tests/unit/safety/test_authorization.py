...
def test_comment_step_status_and_code_write_matrix():
    ...
    assert authorize(ActionRequest(action="write_code"), AuthorizationContext(codeWriteEnabled=True, routingUnique=True, repositoryGuardPassed=True, snapshotStable=True)).allowed
...
def test_unstable_comment_requires_exact_current_turn_record():
    ...
    assert authorize(
        action,
        AuthorizationContext(**common, authorizationRecords=(approved,)),
    ).allowed
...
def test_unstable_code_write_requires_exact_bug_action_record():
    ...
    assert authorize(
        action,
        AuthorizationContext(**base, authorizationRecords=(approved,)),
    ).allowed
...
def test_authorization_compares_every_field_and_rejects_prior_turn_or_non_user_source():
    ...
    for changed in (
        ...
        base.model_copy(update={"action": "resolve_bug"}),
