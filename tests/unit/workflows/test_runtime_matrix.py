...
class RecordingProvider:
    ...
    def query_my_bugs(self, *, scope_names: tuple[str, ...], page: int = 1, page_size: int = 20) -> BugPage:
        ...
        return BugPage(items=pages.get(page, ()), coverage=Coverage(page=page, pageSize=2, total=5))

    def query_user_bugs(self, user: str, *, scope_names: tuple[str, ...], page: int = 1, page_size: int = 20) -> BugPage:
        self.list_calls.append((user, scope_names, page, page_size))
        values = {("alice", 1): (1, 2), ("alice", 2): (3,), ("bob", 1): (2, 4), ("bob", 2): (5,)}
        return BugPage(items=tuple(bug(x) for x in values.get((user, page), ())), coverage=Coverage(page=page, pageSize=2, total=3))
...
def test_per_bug_failure_continues_and_payload_records_partial_result() -> None:
    ...
    assert [(item.bugId, item.category) for item in result.failures] == [("2", "ValueError")]
    assert result.completeness == "PARTIAL"
...
def test_outer_fatal_releases_and_readonly_modes_never_write_provider() -> None:
    ...
    with pytest.raises(RuntimeError, match="sink unavailable"):
        run_team_report(make_context(provider, ledger, sink=RecordingSink(fatal=True), dry_run=True, readonly=True))
    assert ledger.releases == [("lease", RunStatus.FAILED)]
